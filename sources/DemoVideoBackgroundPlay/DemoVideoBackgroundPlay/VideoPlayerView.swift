...
class PlayerManager: NSObject, ObservableObject {
  ...
  func setupPlayer(with url: URL) {
    ...
    print(#function)
    defer {
      print("🎬 创建播放器耗时: \(Date().timeIntervalSince(date)) 秒")
    }
    ...
    observeTimeControlStatus(for: newPlayer)
...
//    }
    ...
    private func addTimeObserver() {
      ...
      timeObserver = player?.addPeriodicTimeObserver(forInterval: interval, queue: .main) { [weak self] _ in
        ...
        if let player = self?.player {
          self?.updateCoverVisibility(for: player)
        }
      }
    }
  ...
  private func observeTimeControlStatus(for player: AVPlayer) {
    timeControlStatusObserver = player.observe(\.timeControlStatus, options: [.initial, .new]) { [weak self] observedPlayer, _ in
      self?.updateCoverVisibility(for: observedPlayer)
    }
  }
  ...
  private func updateCoverVisibility(for player: AVPlayer?) {
    ...
    let progressed = player.currentTime().seconds > 0 && player.currentTime().seconds.isFinite

    let shouldHideCover = itemReady && isPlaying && (keepUp || progressed)

    DispatchQueue.main.async { [weak self] in
      self?.isReadyToPlay = shouldHideCover
    }
  }
...
struct VideoPlayerView: View {
  ...
  init(videoURL: URL, playerManager: PlayerManager = PlayerManager.shared) {
  ...
  var body: some View {
    #if os(iOS)
    AVPlayerViewControllerRepresented(player: playerManager.player!)
