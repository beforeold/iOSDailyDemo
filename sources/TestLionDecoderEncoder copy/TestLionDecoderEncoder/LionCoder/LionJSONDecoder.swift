...
open class JSONDecoder {
    ...
    /// configuration provider used for customizing, be LionJSONDecodableDefaultConfigurationProvider by default
    open var lion_decodingConfigurationProvider: LionJSONDecodableConfigurationProviding.Type? = LionJSONDecodableDefaultConfigurationProvider.self
    ...
    open func decode<T: Decodable>(_ type: T.Type, from data: Data) throws -> T {
        do {
            ...
            let decoder = JSONDecoderImpl(
                ...
                lion_associatedDecodingType: T.self,
                lion_decodingConfigurationProvider: self.lion_decodingConfigurationProvider
            )
...
fileprivate struct JSONDecoderImpl {
    ...
    let lion_associatedDecodingType: Any.Type?
    let lion_decodingConfigurationProvider: LionJSONDecodableConfigurationProviding.Type?

    init(userInfo: [CodingUserInfoKey: Any],
         from json: JSONValue,
         codingPath: [CodingKey],
         options: JSONDecoder._Options,
         lion_associatedDecodingType: Any.Type?,
         lion_decodingConfigurationProvider: LionJSONDecodableConfigurationProviding.Type?) {
        self.userInfo = userInfo
        self.codingPath = codingPath
        self.json = json
        self.options = options
        self.lion_associatedDecodingType = lion_associatedDecodingType
        self.lion_decodingConfigurationProvider = lion_decodingConfigurationProvider
    }
}
...
extension JSONDecoderImpl: Decoder {
    ...
    private func unwrapDictionary<T: Decodable>(as: T.Type) throws -> T {
        ...
        for (key, value) in object {
            ...
            let newDecoder = JSONDecoderImpl(userInfo: self.userInfo,
                                             from: value,
                                             codingPath: newPath,
                                             options: self.options,
                                             lion_associatedDecodingType: dictType.elementType,
                                             lion_decodingConfigurationProvider: self.lion_decodingConfigurationProvider)
        ...
        as type: T.Type) throws -> T
    {
        ...
        if case .number(let number) = value {
            guard let floatingPoint = T(number), floatingPoint.isFinite else {
                ...
                if let provider = self.lion_decodingConfigurationProvider {
                    return provider.guardFloatingPoint(from: self, value: value, for: additionalKey, as: type)
                }
                throw DecodingError.dataCorrupted(.init(
                    codingPath: path,
    ...
    private func lion_mapStringForBool(_ bool: Bool) -> String {
        return bool ? "1" : "0"
    }
    
    private func lion_mapStringForNull() -> String {
        return "0"
    }
    
    static let trueStrings = Set<String>(arrayLiteral: "true", "True", "TRUE", "1")
    static let falseStrings = Set<String>(arrayLiteral: "false", "False", "FALSE", "0")
    private func safeNumberStringForString(_ string: String) -> String {
        if Self.trueStrings.contains(string) {
            return "1"
        }
        
        if Self.falseStrings.contains(string) {
            return "0"
        }
    ...
    /// transform bool/string/null into number case
    /// - Parameter value: the json value to be mapped
    /// - Returns: the mapped json value, or nil if no need
    private func lion_mapToNumber(_ value: JSONValue) -> JSONValue? {
        if case .bool(let bool) = value {
            return .number(lion_mapStringForBool(bool))
        } else if case .string(let string) = value {
            return .number(safeNumberStringForString(string))
        } else if case .null = value {
            return .number(lion_mapStringForNull())
        }
    ...
    struct KeyedContainer<K: CodingKey>: KeyedDecodingContainerProtocol {
        ...
        func decode(_ type: Bool.Type, forKey key: K) throws -> Bool {
            guard let value = dictionary[key.stringValue] else {
                if let provider = self.impl.lion_decodingConfigurationProvider {
                    return provider.guardBool(from: self.impl, value: nil, for: key)
                }
                throw createKeyNotFoundError(forKey: key)
            }
            
            guard case .bool(let bool) = value else {
                if let provider = self.impl.lion_decodingConfigurationProvider {
                    return provider.guardBool(from: self.impl, value: value, for: key)
                }
        ...
        func decode(_ type: String.Type, forKey key: K) throws -> String {
            guard let value = dictionary[key.stringValue] else {
                if let provider = self.impl.lion_decodingConfigurationProvider {
                    return provider.guardString(from: self.impl, value: nil, for: key)
                }
            ...
            guard case .string(let string) = value else {
                if let provider = self.impl.lion_decodingConfigurationProvider {
                    return provider.guardString(from: self.impl, value: value, for: key)
                }
        ...
        func decode<T>(_ type: T.Type, forKey key: K) throws -> T where T: Decodable {
            let newDecoder = try decoderForKey(key, originalType: T.self)
        ...
        private func decoderForKey<LocalKey: CodingKey>(_ key: LocalKey, originalType: Any.Type? = nil) throws -> JSONDecoderImpl {
            let value = try getValue(forKey: key)
            var newPath = self.codingPath
            newPath.append(key)
            ...
            let decoder = JSONDecoderImpl(
                ...
                lion_associatedDecodingType: self.impl.lion_associatedDecodingType,
                lion_decodingConfigurationProvider: self.impl.lion_decodingConfigurationProvider
            )
            return decoder
        }
        ...
        @inline(__always) private func createKeyNotFoundError<LocalKey: CodingKey>(forKey key: LocalKey) -> DecodingError {
            return DecodingError.keyNotFound(key, .init(
                codingPath: self.codingPath,
        ...
        @inline(__always) private func decodeFixedWidthInteger<T: FixedWidthInteger>(key: Self.Key) throws -> T {
            // lion_mark
            let value: JSONValue = (try? getValue(forKey: key)) ?? .null
            guard let ret = try? self.impl.unwrapFixedWidthInteger(from: value, for: key, as: T.self) else {
                return 0
            }
            return ret
        }
        ...
        @inline(__always) private func decodeFloatingPoint<T: LosslessStringConvertible & BinaryFloatingPoint>(key: K) throws -> T {
            guard let value = dictionary[key.stringValue] else {
                if let provider = self.impl.lion_decodingConfigurationProvider {
                    return provider.guardFloatingPoint(from: self.impl, value: nil, for: key, as: T.self)
                }
                throw createKeyNotFoundError(forKey: key)
...
extension JSONDecoderImpl {
    struct UnkeyedContainer: UnkeyedDecodingContainer {
        ...
        init(impl: JSONDecoderImpl, codingPath: [CodingKey], array: [JSONValue]) {
            ...
            // lion_mark
            self.array = array.filter { $0 != .null }
