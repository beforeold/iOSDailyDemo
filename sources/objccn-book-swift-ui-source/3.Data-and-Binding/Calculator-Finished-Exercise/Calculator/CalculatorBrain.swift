...
enum CalculatorBrain {
  ...
  var output: String {
    ...
    case .result(let string): result = string
  ...
  private func apply(num: Int) -> CalculatorBrain {
    ...
    case .result: return .left("0".apply(num: num))
    case .error:
  ...
  private func applyDot() -> CalculatorBrain {
    ...
    case .result(let string): return .left(string.applyDot())
    case .error:
  ...
  private func apply(command: CalculatorButtonItem.Command) -> CalculatorBrain {
    ...
    case .percent:
      ...
      case .result(let string):
        return .left(string.percentaged())
      case .error:
        return .left("-0")
      }
    }
